"""Qwen2D VAE architecture for Comfy custom-node loading."""
    ...
    def __init__(self, dim, mode):
        ...
        if mode == "upsample2d":
            self.resample = nn.Sequential(
                QwenImageUpsample2D(scale_factor=(2.0, 2.0), mode="nearest-exact"),
...
class QwenImageResidualBlock2D(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.0):
        ...
        self.conv_shortcut = ops.Conv2d(in_dim, out_dim, 1) if in_dim != out_dim else nn.Identity()
...
class Qwen2DVAE(nn.Module):
    def __init__(
        ...
        self.temperal_upsample = list(self.temperal_downsample[::-1])
    ...
    def _restore_frames(self, x, frame_info):
...
AutoencoderKLQwenImage2D = Qwen2DVAE
